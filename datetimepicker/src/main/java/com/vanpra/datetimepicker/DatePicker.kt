...
internal fun DatePickerLayout(
    ...
    Column(modifier.drawBackground(colors.background)) {
        DateTitle(selectedDate, colors)
        ViewPager(
            Modifier.drawBackground(Color.Transparent),
            useAlpha = true
        ) {
...
@Composable
private fun DaysTitle(colors: ColorPalette) {
    Row(
        horizontalArrangement = Arrangement.SpaceEvenly,
...
@Composable
private fun DateTitle(selected: MutableState<LocalDate>, colors: ColorPalette) {
    val month = selected.value.month.shortLocalName
    val day = selected.value.dayOfWeek.shortLocalName

    Box(backgroundColor = colors.primary, modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Text(
                text = selected.value.year.toString(), color = colors.onPrimary,
